...
const orderScene = new Scenes.WizardScene(
  ...
  // 0. Город
  async (ctx) => {
    if (ctx.message && ctx.message.text === '⬅️ Назад') {
      await ctx.reply('🔙 Возврат в главное меню.', mainMenu);
      return ctx.scene.leave();
    }
    ctx.wizard.state.data = {};
    await ctx.reply('Выберите город доставки:', Markup.keyboard([['Москва+обл.', 'Санкт-Петербург+обл.'], ['⬅️ Назад']]).oneTime().resize());
  ...
  async (ctx) => {
    ...
    await ctx.reply('Что требуется?', Markup.keyboard([['Бетон', 'Раствор'], ['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    if (type === 'Раствор') {
      ...
      await ctx.reply('Введите марку материала (например, М100):', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
      return ctx.wizard.selectStep(4);
    }

    const city = ctx.wizard.state.data.city;
    if (city === 'Москва+обл.') {
      await ctx.reply('Выберите тип наполнителя:', Markup.keyboard([['Гранит', 'Гравий'], ['⬅️ Назад']]).oneTime().resize());
      return ctx.wizard.next();
    } else {
      ctx.wizard.state.data.fillerType = 'Гранит';
      await ctx.reply('Введите марку материала (например, М300):', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
      return ctx.wizard.selectStep(4);
    }
  },
  ...
  async (ctx) => {
    ...
    await ctx.reply('Укажите объём в м³:', Markup.keyboard([['Помощь в расчёте'], ['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    if (text === 'Помощь в расчёте') {
      ...
      await ctx.reply('Введите длину в метрах:', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
    ...
    await ctx.reply('Как хотите указать адрес?', Markup.keyboard([['Ввести вручную', 'Отправить геолокацию'], ['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    if (choice === 'Ввести вручную') {
      await ctx.reply('Введите адрес доставки:', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
      return ctx.wizard.selectStep(8);
    }
    if (choice === 'Отправить геолокацию') {
      await ctx.reply('Отправьте геопозицию через кнопку 📎.', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
  ...
  async (ctx) => {
    ...
    await ctx.reply('Укажите дату и время доставки:', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
  ...
  async (ctx) => {
    ...
    await ctx.reply('Способ подачи:', Markup.keyboard([['Самослив', 'Автобетононасос'], ['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    if (method === 'Автобетононасос') {
      await ctx.reply('Укажите длину стрелы:', Markup.keyboard([['22м', '24м', '28м', '32м', '36м', '40м', '52м'], ['⬅️ Назад']]).oneTime().resize());
      return ctx.wizard.next();
    }
    ctx.wizard.state.data.pumpLength = 'Не требуется';
    await ctx.reply('Вы физлицо или юрлицо?', Markup.keyboard([['Физлицо', 'Юрлицо'], ['⬅️ Назад']]).oneTime().resize());
  ...
  async (ctx) => {
    ...
    await ctx.reply('Способ оплаты:', Markup.keyboard([['Наличный расчёт', 'Безналичный расчёт'], ['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    await ctx.reply('Введите номер телефона (например, +7 999 123-45-67):', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    await ctx.reply('Добавьте комментарий или напишите "нет":', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    await ctx.reply('Теперь введите ширину:', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    await ctx.reply('Теперь введите высоту:', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    await ctx.reply('Использовать этот объём?', Markup.keyboard([['✅ Да', '❌ Нет, ввести вручную'], ['⬅️ Назад']]).oneTime().resize());
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    if (answer === '❌ Нет, ввести вручную') {
      await ctx.reply('Введите объём в м³:', Markup.keyboard([['⬅️ Назад']]).oneTime().resize());
